...
def Gx_convert(fitness1,gx_data_select):
    ...
    return fitness2
#用于GA_forDNN中的交叉变异
def crossover_and_mutation_GA_for_DNN(pop2,num_var,num_room_type,CROSSOVER_RATE,MUTATION_RATE):
    pop = pop2
    ...
    for i in range(len(pop)):
        ...
        if np.random.rand() < CROSSOVER_RATE:
            ...
            for j in range(min(exchan),max(exchan)):
                child[j] = mother[j]
        mutation_GA_for_DNN(child,num_var,num_room_type,MUTATION_RATE)
        new_pop[i] = child
...
#用于GA_forDNN中的交叉变异
def mutation_GA_for_DNN(child,num_var,num_room_type,MUTATION_RATE):
    num_var = int(num_var)
    ...
    for j in range(num_var+num_room_type):
        if np.random.rand() < MUTATION_RATE:
            child[j] = randint(1,3)
...
def DNN_GA(num_var,num_room_type,num_ind,best_indivi,run_time,model):
    ...
    #verbose取消打印损失
    model.fit(x_train_local, y_train_local, epochs=1200, batch_size=32,verbose=0)#训练模型
    ...
    history=model.fit(x_train, y_train, epochs=1200, batch_size=32,verbose=0)#训练模型
...
def GA_DNN_run(ModelPath_name,mySapObject_name,SapModel_name,num_var,num_room_type,x,labels,time):
    ...
    for run_time in range(N_GENERATIONS):
        ...
        run_time +=1
        if run_time % 20 == 0:
            pop2_new,model = DNN_GA(num_var,num_room_type,int(1 * len(pop2)),pop2[0],200,model)
